...
def generate_sys_cfg():
    ...
    model_path = filedialog.askopenfilename(
        ...
        initialdir="/app/data/model",
        filetypes=[("ONNX files", "*.onnx"), ("All files", "*.*")],
    )
    external_drive_path = filedialog.askdirectory(
        title="Select RAW_DATA directory", initialdir="/media/data/RAW_DATA"
    )
    results_dir = filedialog.askdirectory(
        title="Select PROCESSED_DATA directory", initialdir="/media/data/PROCESSED_DATA"
    )
    gpu = tk.messagebox.askyesno(
        "GPU Usage", "Do you want to use GPU for inference if available?"
    )
...
class InfConfig:
    def __init__(
        ...
        results_dir: str = ".",
        input_shape: Optional[tuple] = (256, 256),
        cam_number: Optional[int] = 1,
        nth_image: Optional[int] = None,  # only inf every nth image
        ...
        if survey_dir:
            self.data = pd.read_csv(
                os.path.join(
                    survey_dir, "cam_{}".format(self.cam_number), "photo_log.csv"
                )
            )
        ...
        self.results_dir = os.path.join(
            results_dir, "cam_{}".format(self.cam_number), ts
        )
...
class Inference:
    def __init__(self, config: InfConfig, inference_only: bool = False):
        self.config = config
        ...
        if not self.inference_only:
            with open(os.path.join(self.results_dir, "inference_config.txt"), "w") as f:
                for key, value in self.config.__dict__.items():
                    if (
                        key != "model" or key != "data"
                    ):  # don't write the model or data to file
                        f.write(f"{key}: {value}\n")
    ...
    def preprocess(self, image):
        ...
        for patch in grid:
            ...
            mean = torch.tensor(
                [0.485, 0.456, 0.406], device=self.config.torch_device
            ).view(3, 1, 1)
            std = torch.tensor(
                [0.229, 0.224, 0.225], device=self.config.torch_device
            ).view(3, 1, 1)
    ...
    def preprocess_patch(self, patch):
        # TODO: refactor the normalize code and resize/transformation code into a separate function
        patch = cv2.cvtColor(patch, cv2.COLOR_BGR2RGB)
        patch = np.transpose(patch, (2, 0, 1))
        patch_tensor = torch.from_numpy(patch).to(self.config.torch_device)
        ...
        std = torch.tensor([0.229, 0.224, 0.225], device=self.config.torch_device).view(
            3, 1, 1
        )
        patch_tensor = (patch_tensor - mean) / std
        return patch_tensor.cpu().numpy()
    ...
    def inf_directory(self):
        ...
        geo_df = pd.read_csv(
            photo_log_path, index_col="filename_string", names=photo_log_column_names
        )
        ...
        for file in files:
            ...
            patch_geo_centres = self.compute_patch_coordinates(
                lat,
                lon,
                distance_to_seafloor_m,
                patch_decisions=output,
                grid_x=7,
                grid_y=4,
            )
            ...
            if (
                sum(
                    patch_inference_result[2]
                    for patch_inference_result in patch_geo_centres
                )
                >= 28
            ):
                overall_deploy = "Yes"
            ...
            for i, (p_lat, p_lon, p_dec) in enumerate(patch_geo_centres):
                ...
                newdf.append(
                    new_row
                )  # pd.concat([newdf, pd.DataFrame([new_row])], ignore_index=True)
            pd.DataFrame(newdf).to_csv(
                csvpth, index=False, mode="a", header=not os.path.exists(csvpth)
            )
    ...
    def compute_patch_coordinates(
        self, lat, lon, distance_to_seafloor_m, patch_decisions, grid_x=7, grid_y=4
    ):
        """
        ...
        x_coords = np.linspace(
            -real_width_m / 2 + patch_width / 2,
            real_width_m / 2 - patch_width / 2,
            grid_x,
        )
        y_coords = np.linspace(
            -real_height_m / 2 + patch_height / 2,
            real_height_m / 2 - patch_height / 2,
            grid_y,
        )
        ...
        if len(patch_decisions) != len(centers):
            raise ValueError(
                "Number of patch decisions must match the number of patches."
            )

        extended_centers = [
            (patch_lat, patch_lon, decision)
            for (patch_lat, patch_lon), decision in zip(centers, patch_decisions)
        ]
